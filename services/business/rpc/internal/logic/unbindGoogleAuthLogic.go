...
type UnbindGoogleAuthLogic struct {
	ctx    context.Context
	svcCtx *svc.ServiceContext
	logx.Logger
}

func NewUnbindGoogleAuthLogic(ctx context.Context, svcCtx *svc.ServiceContext) *UnbindGoogleAuthLogic {
	return &UnbindGoogleAuthLogic{
		ctx:    ctx,
		svcCtx: svcCtx,
...
func (l *UnbindGoogleAuthLogic) UnbindGoogleAuth(in *pb.UnbindGoogleAuthReq) (*pb.UnbindGoogleAuthResp, error) {
	if in == nil {
	...
	case pb.UnbindVerifyMethod_UNBIND_VERIFY_METHOD_EMAIL:
		...
		if !l.verifyEmailCode(email, in.EmailCode, "2fa") {
			l.Logger.Info("Email code verification failed for user %d, email=%s, code=%s", uid, email, in.EmailCode)
	...
	// Clear Google Auth secret and backup codes from user account
	...
	if err := userRepo.GetDB().WithContext(l.ctx).Model(&model.UserModel{}).
		...
		l.Logger.Errorf("clear google auth secret failed: %v", err)
		return nil, errx.DBError()
	}

	// Update member_security_setting: unbind GA
	if l.svcCtx.UserSecuritySettingsRepository != nil {
		...
		if err := l.svcCtx.UserSecuritySettingsRepository.SetGoogleAuthBound(l.ctx, uid, false); err != nil {
			return nil, errx.DBError()
		}
	}

	ip := middleware.GetClientIP(l.ctx)
	// Best-effort: record 2FA history (TOTP unbind)
	...
	if err := SetSecurityCooldown(l.ctx, l.svcCtx, uid, CooldownReasonGAUnbind); err != nil {
		l.Logger.Errorf("Failed to set security cooldown for user %d: %v", uid, err)
		// 不阻塞主流程，只记录日志
	}

	l.Logger.Infof("Google Auth unbound successfully for user %d, security cooldown activated", uid)

	return &pb.UnbindGoogleAuthResp{Success: true, Message: "ok"}, nil
}

// verifyEmailCode verifies email verification code
func (l *UnbindGoogleAuthLogic) verifyEmailCode(email, codeStr, scene string) bool {
...
func (l *UnbindGoogleAuthLogic) verifySMSCode(phone, codeStr, scene string) bool {
	length := l.svcCtx.Config.Code.Length
	if length <= 0 {
		length = 6
	}
...
func (l *UnbindGoogleAuthLogic) verifyBiometricToken(userID int64, tokenStr string) bool {
...
func (l *UnbindGoogleAuthLogic) checkUserHasBiometricKey(userID int64) bool {
...
func (l *UnbindGoogleAuthLogic) getBiometricSecretKey(userID int64) string {
