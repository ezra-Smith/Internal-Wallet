...
type ListDepositRecordsLogic struct {
	ctx    context.Context
	svcCtx *svc.ServiceContext
	logx.Logger
}

func NewListDepositRecordsLogic(ctx context.Context, svcCtx *svc.ServiceContext) *ListDepositRecordsLogic {
	return &ListDepositRecordsLogic{
		ctx:    ctx,
		svcCtx: svcCtx,
...
func (l *ListDepositRecordsLogic) ListDepositRecords(in *pb.ListDepositRecordsReq) (*pb.ListDepositRecordsResp, error) {
	...
	items := []*pb.DepositRecordItem{}

	if l.svcCtx.AccountingRpc == nil {
		return &pb.ListDepositRecordsResp{Success: true, Total: 0, Items: items}, nil
	}
	...
	for chainCode := range chainSet {
		...
		// Store chain for explorer URL building later
		if chain != nil {
	...
	for _, r := range accResp.GetItems() {
		...
		// Determine confirmations (for success, show required; for pending, could show current)
		...
		items = append(items, &pb.DepositRecordItem{
			...
			AmountDisplay:         FormatAmountDisplay(r.AmountDecimal, asset, true),
	...
	return &pb.ListDepositRecordsResp{Success: true, Total: int32(accResp.Total), Items: items}, nil
}
