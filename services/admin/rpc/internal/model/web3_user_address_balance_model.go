...
// 注意：已删除 web3_user_address_id 和 web3_user_id 字段，余额记录是地址级别的，不绑定到特定用户/设备
...
type Web3UserAddressBalanceModel struct {
	...
	BalanceUpdatedAt *time.Time `gorm:"column:balance_updated_at;type:timestamp(6)"`
	LastSyncedAt     *time.Time `gorm:"column:last_synced_at;type:timestamp(6)"`
	SyncStatus       string     `gorm:"column:sync_status;type:varchar(20);not null;default:'unknown'"`
